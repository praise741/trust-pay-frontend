...
import { Search, ArrowLeftRight, Loader2, Filter } from "lucide-react";
...
export default function SellerTransactionsPage() {
  ...
  const filtered = deals.filter((t) => {
    ...
    const matchSearch = desc.toLowerCase().includes(searchQuery.toLowerCase()) || buyer.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-md"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Search transactions..." className="pl-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
